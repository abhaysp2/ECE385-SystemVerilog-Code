...
void KeyExpansion(unsinged char * key_schedule, unsigned char * key)
{
	int i = 0;
	int k = 0;
	unsigned char temp[4];
	while (i < 16){
		key_schedule[(4*i)] = key[(4*i)];
		key_schedule[(4*i)+1] = key[(4*i)+1];
		key_schedule[(4*i)+2] = key[(4*i)+2];
		key_schedule[(4*i)+3] = key[(4*i)+3];
		i = i + 1;
	}
	i = 4;
	while (i < 44){
		temp = {key_schedule[4*(i-1)], key_schedule[4*(i-1)+1], key_schedule[4*(i-1)+2], key_schedule[4*(i-1)+3]};
		if(i % 4 == 0){
...
void AddRoundKey(unsigned char * key_schedule, unsigned char * state)
{
	int i = 0;
	for(i = 0; i < 16; i++){
...
void MixColumns(unsinged char * state)
{
	uchar temp[16];
	int i;
	for(i = 0; i < 16; i++){
...
void ShiftRows(unsinged char * state)
{
